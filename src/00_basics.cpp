...
int main_00() {
    ...
    // -----------------------------
    StaticShader basic_shader;
    if (basic_shader.getStatus() != BaseShader::link_prog_passed || basic_shader.getProgId() <= 0) {
    ...
    basic_shader.start();
    basic_shader.loadProjMatrix(renderer.getProjMatrix());
    basic_shader.stop();
    ...
    while ( win.isValid() ) {
        ...
        if (1) {
            renderer.prepare();    
                basic_shader.start();
                basic_shader.loadViewMatrix(renderer.getViewMatrix());

                    for (auto itr = assimp.entities.begin(); itr != assimp.entities.end(); itr++) {
                        renderer.render(&(*itr), &basic_shader);  // proj, view matrices has been loaded into shader before, 
                                                                  // transform matrix is loaded into shader here.
                    }

                    renderer.render(&multi_vbo_entity, &basic_shader);
                    renderer.render(&single_vbo_entity, &basic_shader);
