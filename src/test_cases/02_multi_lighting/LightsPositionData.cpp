...
LightsPositionData::LightsPositionData() {
    move_type = LightsPositionData::rotate;
    curr_rotz = 0.0f;
    
    float init_position[NUM_LIGHTS][Light::Position::max_pos] = {
        { 0.0f,  2.0f, 1.8f},
        {-2.0f,  0.0f, 1.8f},
        { 0.0f, -2.0f, 1.8f},
        { 2.0f,  0.0f, 1.8f}
    ...
    for (unsigned int i = 0; i < NUM_LIGHTS; i++) {
        for (unsigned int j = 0; j < Light::Position::max_pos; j++) {
            position[i][j] = init_position[i][j];
        }
    }
}

void LightsPositionData::update() {
    ...
    switch (move_type) {
        case rotate:
        {
            ...
            if (curr_rotz > 3.14f / 4.0f && curr_rotz < 3.14f / 4.0f + rot_z_step) {
                move_type = crate0_get_closer;
            }
            if (curr_rotz > 3.14f / 2.0f + 3.14f / 4.0f && curr_rotz < 3.14f / 2.0f + 3.14f / 4.0f + rot_z_step) {
                move_type = crate3_get_closer;
            }
            if (curr_rotz > 3.14f + 3.14f / 4.0f && curr_rotz < 3.14f + 3.14f / 4.0f + rot_z_step) {
                move_type = crate2_get_closer;
            }
            if (curr_rotz > 3.14f + 3.14f / 2.0f + 3.14f / 4.0f && curr_rotz < 3.14f + 3.14f / 2.0f + 3.14f / 4.0f + rot_z_step) {
