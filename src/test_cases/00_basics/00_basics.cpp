...
#include "test_cases/00_basics/BasicRenderer.h"
...
int test_00_basics() {
    ...
    AssimpLib assimp_misa; {
        ...
        //TODO: support loading with a single-transform each time in addition to 
        //              loading with an array-transforms each time
    ...
    while ( win.isValid() ) {
        ...
        if (1) {
            renderer.prepare();
                for (auto itr = assimp_misa.entities.begin(); itr != assimp_misa.entities.end(); itr++) {
                    renderer.process(&(*itr));  // proj, view matrices has been loaded into shader before, 
                                                // transform matrix is loaded into shader here.
                }
                renderer.process(&multi_vbo_entity);
                renderer.process(&single_vbo_entity);
